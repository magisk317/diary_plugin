...
class DiaryStorage:
    ...
    async def save_diary(self, diary_data: Dict[str, Any], expected_hour: int = None, expected_minute: int = None) -> bool:
        ...
        try:
            ...
            if expected_hour is not None and expected_minute is not None:
                filename = f"{self._format_date_str(date)}_{expected_hour:02d}{expected_minute:02d}00.json"
            else:
                timestamp = datetime.datetime.fromtimestamp(generation_time)
                filename = f"{self._format_date_str(date)}_{timestamp.strftime('%H%M%S')}.json"
    ...
    def _format_date_str(self, date_input: Any) -> str:
        """格式化日期字符串"""
    ...
    async def get_diary(self, date: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for filename in os.listdir(self.data_dir):
                if filename.startswith(f"{self._format_date_str(date)}_") and filename.endswith('.json'):
                    file_path = os.path.join(self.data_dir, filename)
